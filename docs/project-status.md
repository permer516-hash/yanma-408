...
- 已创建 `apps/web`。
- 题库页已完成筛选与搜索加载优化：筛选直接请求数据并轻量同步 URL，文本搜索采用 `300ms` 防抖，请求期间保留旧题目和分页信息；浏览器检查链路中的科目切换由约 `3.1s` 降至约 `332ms`。
- 已完成前端视觉与布局第二阶段升级：在第一阶段首页、题库和管理后台基础上，继续统一错题本、学习分析、练习作答、套卷详情/作答/报告、教师学情、师生绑定和账号安全页面；本次仅调整视觉层级与响应式布局，未改变接口、权限和业务流程。
...
- 前端已完成第一阶段视觉与布局升级：
  - 全局补齐统一的页面背景、面板、按钮、列表、表格头和输入焦点态样式。
  - 首页仪表盘、题库页和管理后台已从原型式页面升级为更专业的产品化界面。
  - 首页移动端导航已压缩为双列布局，减少手机首屏占用。
- 当前页面包含：
  - 左侧导航
  - 今日学习仪表盘
  - 考研倒计时
